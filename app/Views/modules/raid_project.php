        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body">
                ...
                <form method="post" action="<?= site_url('projects/' . (int) ($project['id'] ?? 0) . '/modules/' . $moduleRouteSegment) ?>" class="row g-2">
                    <?= csrf_field() ?>
                    <div class="col-12 col-md-4">
                        <label class="form-label" for="title"><?= esc(lang('Module.raidColumnTitle')) ?></label>
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <select id="owner_user_id_create" name="owner_user_id" class="form-select" required>
                            <?php foreach ($owners as $owner): ?>
                    ...
                    <div class="col-6 col-md-2">
                        ...
                        <select id="status_create" name="status" class="form-select" required>
                            <?php foreach ($statusOptions as $statusOption): ?>
                                    ...
                                    <td>
                                        <details>
                                            ...
                                            <form method="post" action="<?= site_url('projects/' . (int) ($project['id'] ?? 0) . '/modules/' . $moduleRouteSegment . '/' . (int) ($entry['id'] ?? 0) . '/update') ?>" class="d-grid gap-2">
                                                <?= csrf_field() ?>
                                                <input name="title" type="text" class="form-control form-control-sm" maxlength="200" required value="<?= esc((string) ($entry['title'] ?? '')) ?>">
                                                ...
                                                <select name="status" class="form-select form-select-sm" required>
                                                    <?php foreach ($statusOptions as $statusOption): ?>
                                                        <option value="<?= esc($statusOption) ?>" <?= (string) ($entry['status'] ?? '') === $statusOption ? 'selected' : '' ?>><?= esc((string) lang('Module.raidStatus' . ucfirst($statusOption))) ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                                <select name="priority" class="form-select form-select-sm" required>
                                                    <?php foreach ($priorityOptions as $priorityOption): ?>
                                                        <option value="<?= esc($priorityOption) ?>" <?= (string) ($entry['priority'] ?? '') === $priorityOption ? 'selected' : '' ?>><?= esc((string) lang('Module.raidPriority' . ucfirst($priorityOption))) ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                                <input name="target_date" type="date" class="form-control form-control-sm" value="<?= esc((string) ($entry['target_date'] ?? '')) ?>">
                                                <input name="review_date" type="date" class="form-control form-control-sm" value="<?= esc((string) ($entry['review_date'] ?? '')) ?>">
                                                <button class="btn btn-sm btn-primary" type="submit"><?= esc(lang('Module.raidUpdateButton')) ?></button>
                                            </form>
                                            <?php if ((string) ($entry['status'] ?? '') !== 'closed'): ?>
