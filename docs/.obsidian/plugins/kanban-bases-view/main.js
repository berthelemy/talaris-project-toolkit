...
var KanbanView = class extends import_obsidian2.BasesView {
  ...
  renderCardTitle(titleEl, entry, filePath) {
    ...
    if (!titleValue || titleValue instanceof import_obsidian2.NullValue) {
      titleEl.textContent = entry.file.basename;
      return;
    }
  ...
  renderCardCover(coverEl, entry, filePath) {
    ...
    if (!value || value instanceof import_obsidian2.NullValue) return false;
  ...
  createCard(entry) {
    ...
    for (const propertyId of order) {
      ...
      if (!value || value instanceof import_obsidian2.NullValue) continue;
  ...
  async createQuickAddCard(title, columnValue, swimlaneValue) {
    ...
    const hasConfiguredFolder = typeof rawConfigFolder === "string" && !!(0, import_obsidian2.normalizePath)(rawConfigFolder.trim());
    if (hasConfiguredFolder && targetFolder && !this.app?.vault.getFolderByPath(targetFolder)) {
      new import_obsidian2.Notice(`Quick add folder not found: ${targetFolder}`);
      return;
    }
    const fileNameToCreate = targetFolder ? (0, import_obsidian2.normalizePath)(`${targetFolder}/${baseFileName}`) : baseFileName;
  ...
  closeNativeNewItemPopover() {
    const closePopovers = () => {
      const popovers = Array.from(this.containerEl.doc.querySelectorAll(".bases-new-item-popover"));
    ...
    if (oldLaneValue === newLaneValue && oldColumnValue === newColumnValue) {
      if (sortActive) {
        if (this.didSortableIndexChange(evt)) {
          new import_obsidian2.Notice(SORTED_CARD_ORDER_NOTICE, 4e3);
        }
        this.render();
        return;
      }
